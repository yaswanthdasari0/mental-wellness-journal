...
export function calcStreak(completedDates: (Date | string)[]): number {
  const done = new Set(
    completedDates.map((d) => dayKey(new Date(d)))
  );

  const cursor = new Date();

  // Today not ticked yet? The streak is still alive, so start from yesterday.
...
export default function HabitProgress({
  ...
  const maxStreak = habits.reduce(
    (max, h) =>
      Math.max(max, h.longestStreak ?? h.streak ?? 0),
    0
  );
          ...
          <>
            <div>
              ...
              <div className="hp-progress-header">
                ...
                <span className="hp-progress-pct">
                  {pct}%
                </span>
              </div>
